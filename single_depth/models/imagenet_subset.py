...
class ResNetBase(nn.Module):
    def __init__(
        ...
        ########
        self.fc_sup = FinalBlock(opt=opt, in_channels=512 * block.expansion, hierarchy=0)
        self.fc_sub = FinalBlock(opt=opt, in_channels=512 * block.expansion, hierarchy=1)
        ########
    ...
    def _forward_impl(self, x, return_feat=False):
        ...
        if return_feat:
            return l_sup, l_sub, x
        else:
            return l_sup, l_sub        

    def forward(self, x, return_feat=False):
        return self._forward_impl(x, return_feat=return_feat)
