...
class ImageDataset(Dataset):
    def __init__(self, root, data_frame: pd.DataFrame, dataset: str, transform=None, cls_list=None, data_dir=None,
                 preload=False, device=None, transform_on_gpu=False, n_classes_sup=None, hierarchy_list=None, depth=0):
        ...
        if 'cifar100' in dataset:
            ...
            raw_data='imagenet'
        elif 'cub_200_2011' in dataset:
            raw_data='cub_200_2011'
        elif 'inat19' in dataset:
            raw_data = 'inat19'
        else:
        ...
        if self.preload:
            ...
            for idx in range(len(self.data_frame)):
                ...
                for h in range(self.depth+1):
                    sample['cls_hier'].append( self.data_frame.iloc[idx][f"label_{h}"] )                

                is_not_hier = torch.LongTensor(self.hierarchy_list != sample['hierarchy'])
                sample["label"] -= is_not_hier[:sample["label"]].sum()        
    ...
    def get_data_gpu(self, indices):
        ...
        for i in indices:
            ...
            cls_hier.append(self.loaded_images[i]["cls_hier"])
            
            
        data['image'] = torch.stack(images)
        data['label'] = torch.LongTensor(labels)            
        data['hierarchy'] = torch.LongTensor(hierarchy)            
        data['cls_hier'] = torch.LongTensor(cls_hier)            

        return data


class StreamDataset(Dataset):
    def __init__(self, root, datalist, dataset, transform, cls_list, data_dir=None, device=None, transform_on_gpu=False, n_classes_sup=None, \
                 hierarchy_list=None, depth=0, save_test=None, test_transform=None):
        ...
        self.cls_sup = []
        self.cls_sub = []
        self.cls_hier = [[] for i in range(self.depth+1)]
        ...
        for data in datalist:
            ...
            self.labels_w.append(self.cls_list.index(data['klass']))
            self.hierarchy.append(data['hierarchy'])
            
            
            #print(self.hierarchy_list)
            #print(data['hierarchy'])
            
            is_not_hier = (torch.tensor(self.hierarchy_list) != data['hierarchy']).type(torch.float64)
            self.labels[-1] -= is_not_hier[:self.labels[-1]].sum()                
            
            for h in range(self.depth+1):
                self.cls_hier[h].append(data[f'label_{h}'])
    ...
    def get_data(self):
        ...
        cls_hier = [[] for i in range(self.depth+1)]
        ...
        for i, image in enumerate(self.images):
            ...
            labels.append(int(self.labels[i]))
            labels_w.append(int(self.labels[i]))
            hierarchy.append(self.hierarchy[i])
            
            for h in range(self.depth+1):
                cls_hier[h].append(self.cls_hier[h][i])

        
        data['image'] = torch.stack(images)
        data['label'] = torch.LongTensor(labels)
        data['label_w'] = torch.LongTensor(labels_w)
        data['hierarchy'] = torch.LongTensor(hierarchy)
        data['cls_hier'] = torch.LongTensor(cls_hier)
    ...
    def get_two_aug_data(self, labels_only=False):
        ...
        for i, image in enumerate(self.images):
            ...
            #print('self.transform_on_gpu: ', self.transform_on_gpu)
            if not labels_only:
                if self.transform_on_gpu:
                    images.append(self.transform_gpu(image.to(self.device)))
                else:                    
...
class MemoryDataset(Dataset):
    def __init__(self, root, dataset, transform=None, cls_list=None, device=None, test_transform=None,
                 data_dir=None, transform_on_gpu=True, save_test=None, keep_history=False, n_classes_sup=None, hierarchy_list=None, depth=1):
        ...
        self.hierarchy_list = []
        self.hierarchy_dict = {cls_list[i]:hierarchy_list[i] for i in range(len(self.hierarchy_list))}
    ...
    def add_new_class(self, cls_list, hierarchy_list):
        self.cls_list = cls_list
        ...
        self.hierarchy_list = hierarchy_list
        self.hierarchy_dict = {self.cls_list[i]:self.hierarchy_list[i] for i in range(len(self.hierarchy_list))}
        
        #print(torch.tensor(hierarchy_list) != hierarchy_list[-1])
        #print(type(hierarchy_list))
        #input()
        
        is_not_hier = (torch.tensor(hierarchy_list) != hierarchy_list[-1]).type(torch.int64)
        #print(is_not_hier)
        
        self.relabels.append(len(self.cls_list) - is_not_hier[:-1].sum() -1  )
    ...
    def replace_sample(self, sample, idx=None):
        ...
        if idx is None:
            ...
            for h in range(self.depth+1):
                self.cls_hier[h].append(sample[f'label_{h}'])
    ...
    def get_batch(self, batch_size, use_weight=False, transform=None, indices=None):
        ...
        for i in indices:
            ...
            is_not_hier = (torch.tensor(self.hierarchy_list) != hierarchy[-1]).type(torch.int)
            labels[-1] -= is_not_hier[:labels[-1]].sum()                
    ...
    def get_two_batches(self, batch_size, test_transform):
        ...
        labels = []
        hierarchy = []
        cls_hier = [[] for h in range(self.depth+1)]
        ...
        data_1['cls_hier'] = torch.LongTensor(cls_hier)
        ...
        data_2['cls_hier'] = torch.LongTensor(cls_hier)
...
def get_statistics(dataset: str):
    ...
    assert dataset in [
        ...
        "stanford_car_setup2",
        "cub_200_2011_scenario1",
        "cifar100_scene_topdown",
        "cifar100_scene_bottomup",
        "inat19_scene_topdown",
    ]
    mean = {
        "mnist": (0.1307,),
        "KMNIST": (0.1307,),
        ...
        "cub_200_2011_scenario1": (0.485, 0.456, 0.406),
        "cifar100_scene_topdown": (0.5071, 0.4867, 0.4408),
        "cifar100_scene_bottomup": (0.5071, 0.4867, 0.4408),
        "inat19_scene_topdown": (0.454, 0.474, 0.367),
    }
    ...
    std = {
        ...
        "cub_200_2011_scenario1": (0.229, 0.224, 0.225),
        "cifar100_scene_topdown": (0.2675, 0.2565, 0.2761),
        "cifar100_scene_bottomup": (0.2675, 0.2565, 0.2761),
        "inat19_scene_topdown": (0.237, 0.230, 0.249),
    ...
    classes = {
        ...
        "cub_200_2011_scenario1": 238,
        "cifar100_scene_topdown": 100,
        "cifar100_scene_bottomup": 100,
        "inat19_scene_topdown": 1010,
    ...
    classes_sup = {
        ...
        "cub_200_2011_scenario1": 38,
        "cifar100_scene_topdown": 20,
        "cifar100_scene_bottomup": 20,
        "inat19_scene_topdown": 72,
    ...
    in_channels = {
        ...
        "cub_200_2011_scenario1": 3,
        "cifar100_scene_topdown": 3,
        "cifar100_scene_bottomup": 3,
        "inat19_scene_topdown": 3,
    ...
    inp_size = {
        ...
        "cub_200_2011_scenario1": 224,
        "cifar100_scene_topdown": 32,
        "cifar100_scene_bottomup": 32,
        "inat19_scene_topdown": 224,
