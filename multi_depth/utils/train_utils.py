...
from models import mnist, cifar, imagenet, imagenet_subset, cub_200_2011, inat19
...
def select_optimizer(opt_name, lr, model):
    if opt_name == "adam":
        ...
        for fc in model.fc:
            opt.add_param_group({'params': fc.parameters()})
...
def select_model(model_name, dataset, num_classes_sup=1, num_classes_sub=1, num_heads=1, num_classes_hier=None):
    opt = edict(
        {
            "depth": 18,
            "num_classes_sup": 2,
            ...
            "compression": 0.5,
            "num_heads": num_heads,
            "num_classes_hier": num_classes_hier
        }
    )
    ...
    if "mnist" in dataset:
        ...
        model_class = getattr(cub_200_2011, "ResNet")
    elif "inat19" in dataset:
        model_class = getattr(inat19, "ResNet")        
