...
from utils import get_kcg, get_entropy
import scipy

from utils import FixMatchModule as ThresholdingModule
...
class PL_FMS(ER):
    def __init__(
        ...
        self.memory = MemoryDataset(self.root, self.dataset, self.train_transform, self.exposed_classes,
                                    ...
                                    transform_on_gpu=self.gpu_transform, n_classes_sup=self.n_classes_sup, save_test='gpu', keep_history=True)
        
        self.online_iter = 1.0 * self.online_iter
        self.imp_update_period = self.online_iter * self.temp_batchsize
        ...
        self.norm_factor = self.total_samples/(2*5) 
    ...
    def online_train(self, sample, batch_size, n_worker, iterations=1, stream_batch_size=1):
        ...
        if stream_batch_size > 0:
            sample_dataset = StreamDataset(self.root, sample, dataset=self.dataset, transform=self.train_transform,
                                           ...
                                           save_test='gpu', test_transform=self.test_transform)
        ...
        for i in range(iterations):
            ...
            if stream_batch_size > 0:
                ...
                self.stream_data = sample_dataset.get_data_for_test()
                
                if self.n_count_num > self.temp_batchsize:
                    labels = deepcopy(stream_data['label_w'])
                                    
                    cls_iter = torch.tensor(self.memory.iter_encountered_cls)[labels]
                    cls_elapsed_iters = self.n_count_num - cls_iter
                    cls_select_prob = torch.clip(cls_elapsed_iters/(20*self.norm_factor/self.num_learned_class), max=1.0)
    ...
    def samplewise_loss_update(self, ema_ratio=0.9, batchsize=512):
    ...
    def update_memory_hier(self):
        
        for i, sample in enumerate(self.temp_batch):
            hier = sample['hierarchy']
            
            if len(self.memory.images) >= self.memory_size:
                label_count_with_stream = np.array(copy.deepcopy(self.memory.cls_count))
                label_num_thresh = self.memory_size // self.num_learned_class_hier[hier]

                label_count_with_stream[self.exposed_classes.index(sample["klass"])] +=1 
                
                label_frequency = label_count_with_stream-label_num_thresh*(np.array(self.exposed_hierarchies)==hier).astype(int)
